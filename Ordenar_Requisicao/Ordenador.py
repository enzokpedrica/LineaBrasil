...
def requisicaoAcessorios():
    arquivo_posicao = r"C:\git\linea-ordenacao-requisicao\Lista_Map.CSV"
    arquivo_impressao = r"C:\git\linea-ordenacao-requisicao\Ordenar_Requisicao\tabela_imprimir.xlsx"
    arquivo_salvo = r"C:\git\linea-ordenacao-requisicao\Ordenar_Requisicao\teste.TXT"
    ...
    df_merged.to_excel(arquivo_impressao, index=False, engine='openpyxl')

    wb = openpyxl.load_workbook(arquivo_impressao)
    ws = wb.active
    ws.title = "Tabela"
    ...
    wb.save(arquivo_impressao)

    # Abra o arquivo Excel salvo
    subprocess.Popen(['start', 'excel.exe', arquivo_impressao], shell=True)
